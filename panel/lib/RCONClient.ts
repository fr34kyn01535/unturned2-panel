...
export default class RCONClient extends EventEmitter{
    ...
    constructor(port: number, host: string, password: string){
        ...
        this.client.on('data', function(data : string) {
            ...
            if(parsed.length !== 4) return console.log("Could not parse RCON result", data);
            var [text,version,command,args] = parsed;
            if(version != "1") return console.log("Invalid RCON version", version);
            switch(command.toUpperCase()){
                case  "AUTH": console.log("Connected"); break;
                case  "PING": that.send("PONG"); break;
                default: {
