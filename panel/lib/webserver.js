...
class WebServer {
    addRouter(directory) {
        ...
        fs.readdirSync(directory).forEach(function (routerName) {
            if (!routerName.endsWith(".js"))
                return;
            var name = path.parse(routerName).name;
            var fullPath = path.resolve(directory, name);
            var routerFunction = require(fullPath);
            var router = new routerFunction.default();
            var root = router.GetRoot();
            if (!root)
                return console.error("Router " + fullPath + " can't be registered without supplying a root");
    ...
    constructor(domain, httpPort, jwtSecret, steamAPIKey, email, enableSSL = false, httpsPort = 2443) {
        var that = this;
        this.app = express();
        ...
        fs.readdirSync("./plugins").forEach(function (directory) {
            ...
            if (fs.existsSync(viewsDirectory))
                that.app.use('/views/', that.viewHandler(viewsDirectory));
            var routersDirectory = path.resolve("./plugins/", directory, "routers");
            if (fs.existsSync(routersDirectory))
                that.addRouter(routersDirectory);
        });
    }
}
